...
## Status (updated 2026-05-30 — v3, 7.1 in progress)
Phase: **7 of 7** (Experiential Texture Continuity) — implementation STARTED.
Done: 7.0 (L0 overflow fix, 5→7 entries); **7.1 write-policy verbatim-anchor rule** (added to `reference_memory_write_policy.md`). L0 hygiene dropped from 7.1 (Dustin questioned it — not a prerequisite; reframed as optional post-digest cleanup, DEF-7-08).
Next: 7.1 lightweight L2 legibility fix (server output-render) → 7.2 consolidation protection.
...
### 7.2 — Protect experiential obs from weekly consolidation (BEFORE the digest produces data)
Patch `~/.claude/prompts/memory-consolidation.md`: add `narrative`/`relational` to its STORE table; never type-correct / tombstone / lossy-merge experiential obs (supersede only with a *richer* version); resolve the tombstone-marker contradiction (write real content, not `[superseded: …]` markers).
...
## Deferments / risks
- **DEF-7-01 (HIGH, pre-existing):** consolidation tombstone-marker contradiction — 7.2 resolves.
