...
- Context-graph types (v1.2.0): `ConnectedNode` / `ConnectedContextOptions` / `ConnectedContextResult` (the latter carries `cycles: string[][]`) for `getConnectedContext`; `PrecedentMatch` / `FindPrecedentsOptions` / `FindPrecedentsResult` for `findPrecedents`. Both methods are declared on the `GraphStore` interface.
...
### `rank-fusion.ts` (~110 lines)
- Pure ranking math for hybrid search (v1.3.0 Phase A/B) — no DB, no model, unit-testable in Pool 1 (same rationale as `rankAndFloorPrecedents`)
- `fuseRanks<K>(lists)` — weighted Reciprocal Rank Fusion (`score = Σ weight/(60+rank+1)`). Rank-based (not score-based) so BM25/cosine/recency/activation never need cross-scale normalization. `RRF_K = 60` (literature standard, deliberately not configurable)
- `toFtsQuery(raw)` — FTS5 MATCH sanitizer: strips NUL bytes, DROPS tokens containing no letter/digit (they tokenize to empty phrases — probed 2026-07-02: tolerated in AND chains on current SQLite but undocumented, and a lone one matches nothing), then wraps every surviving token in double quotes (embedded `"` doubled) so operators in user input (AND/OR/NOT/NEAR/*/^/-) match literally. User input must NEVER reach MATCH unsanitized. Round-trip-tested against the real FTS5 parser with a hostile corpus
- `activationScore(count, lastAccessedAt, nowMs)` — ACT-R base-level activation approximation `ln(1+n) − 0.5·ln(1+hours)`; -Infinity for never-accessed/sentinel; clamps future timestamps (multi-instance clock skew). Consumed by Phase B's activation rerank list
...
- `clampLimit()`: clamps page size to [1, MAX_PAGE_SIZE] with DEFAULT_PAGE_SIZE fallback; floors fractional input (direct GraphStore callers bypass Zod's `.int()` — a fractional limit would flow into computed KNN parameters)
...
- The separator class includes the **whole C0 control range** (`\u0000-\u001f`), not just `\s` (which only covers `\t\n\v\f\r`). This guarantees a normalized name can never contain `char(31)` (0x1f), which `get_connected_context` uses as its recursive-CTE path/cycle-guard delimiter — a name carrying 0x1f would split a path mid-name and corrupt cycle detection.
...
### `relation-types.ts` (~60 lines)
- Documented core causal/precedent relation vocabulary so "why does this rule exist / what incident caused it" is a relation lookup, not a prose grep.
- `CORE_RELATION_TYPES` — frozen array of the three first-class verbs: `CAUSED_BY` (effect → cause), `PRECEDENT_FOR` (earlier → later), `SUPERSEDES` (newer → older). `CoreRelationType` is the union type.
- `RELATION_TYPE_GUIDANCE` — a human-readable directionality block (the same content as the design spec's Appendix B) that `index.ts` appends to the `create_relations` tool description, so the agent sees the canonical verbs + their from→to direction at call time.
- Convention-only by design (per the smallest-sufficient Q1 decision, DEF-CG-03): `relation_type` is NOT canonicalized at the insert — the vocabulary is guidance, not an enum, and any string is still accepted (the store stays schema-unchanged).
...
### `sqlite-store.ts` (~3700 lines)
- `SqliteStore` class: SQLite backend using `better-sqlite3`
- Implements the `GraphStore` interface
- **Context-graph methods (v1.2.0):** `getConnectedContext(seedEntity, opts)` — bounded multi-hop traversal via a `WITH RECURSIVE` CTE (direction-aware `edges` CTE materialized once per call; per-path char(31)-delimited cycle guard; `GROUP BY node` + `MIN(depth)` collapses multi-path arrivals to the nearest hop). The seed is resolved project-independently (it's the anchor, not a scoped node); reached nodes are hydrated + project/temporal-filtered BEFORE the `maxNodes` cap so `truncated` stays honest. `detectDirectedCycles(relations)` — gray/black-coloring DFS over the returned edges; dedups by node-SET via `JSON.stringify(sorted)` (collision-proof + legible). `findPrecedents(query, projectId, opts)` — KNN over `vec_observations` (mirrors the dedup path), hydrate active/in-scope, then `rankAndFloorPrecedents` (module-level pure helper) gates on the raw-cosine floor, ranks raw-DESC, caps, and rounds for display only. Degrades to empty+flags (no recency fallback) when the model/vec table is unavailable.
- **Schema version tracking**: `schema_version` table with `CHECK(id=1)` single-row constraint. Versions 1–12 tracked. Legacy databases detected via column existence heuristics and assigned an initial version. All migrations run in sequence on startup.
...
- **Relevance mode (v1.3.0 Phase A+B):** `searchNodes(..., orderBy: 'relevance')` branches after the shared `matched_ids` CTE into `searchNodesRanked()` — weighted RRF (via `rank-fusion.ts`) over four entity-level candidate lists: LIKE (recency-ordered, weight 1.0), FTS/BM25 best-per-entity (1.0), vector KNN best-cosine-first (1.0, skipped unless model ready), and ACT-R activation (0.5 — computed ONLY over the union of the other three lists: activation reranks candidates, never nominates; never-accessed entities are excluded rather than tail-ranked so no-history contributes exactly nothing). Top-k only: `nextCursor` always null; store-level guard THROWS on cursor or asOf combined with relevance (rank is query-relative/mutation-unstable; FTS holds current content only) — this guard is the ONLY enforcement point (registerTool's per-field ZodRawShape cannot cross-field-validate); JSONL mirrors the cursor guard. `rankingDegraded?: ('fts'|'vector')[]` surfaces lost candidate lists to the caller (goal A — degradation changes result MEMBERSHIP): set on FTS/vector query failure or model still loading/failed; deliberately NOT set when vector search is configured off. `totalCount` in relevance mode = fused-candidate-union size at collection time (may exceed returned entities under concurrent mutation — documented semantic, not a bug). Default `'recency'` is byte-identical to pre-v1.3.0 behavior.
...
- **`last_accessed_at` access discipline**: `touchEntities()` and `touchEntitiesByName()` private helpers update the timestamp. Called by: `searchNodes`, `openNodes`, `entityTimeline`, and all write operations. NOT called by `readGraph` (bulk reads aren't intentional access). The eviction sweep never touches `last_accessed_at` (uses raw SQL in evict.ts). Since v12, `access_count` increments under the PARITY RULE at exactly these sites (see the v11→v12 migration entry).
...
### `index.ts` (~930 lines)
- Entry point: registers 19 MCP tools via `server.registerTool()`
...
- `get_context_layers` tool returns L0 and L1 observations sorted by importance DESC. Enforces soft token budgets (~100 tokens L0, ~4000 tokens L1). Designed for SessionStart/PostCompact hooks. Supports project scoping and layer filtering.
- `get_summary` tool returns a session-start briefing snapshot: top observations by importance DESC then recency DESC, 5 most recently updated entities with observation counts, and aggregate stats (total entities/observations/relations/projects). `excludeContextLayers` boolean omits L0/L1 observations for dedup with `get_context_layers`. `limit` caps top observations (default 20). Optional `memoryType` parameter filters topObservations and recentEntities by memory_type (stats remain unfiltered).
- `search_nodes` tool accepts optional `memoryType` parameter to filter results to entities with at least one observation of the specified memory_type (e.g., 'decision', 'procedure'). The cursor fingerprint includes memoryType to prevent cross-filter pagination.
- `search_nodes` tool accepts optional `orderBy` (`'recency'` default | `'relevance'`). Relevance = RRF-fused top-k (see sqlite-store bullet); incompatible with `cursor`/`asOf` (store guard throws — cross-field checks can't live in registerTool's per-field ZodRawShape). JSONL backend falls back to recency + `rankingUnavailable: true` in the response (flag is in `PaginatedOutputSchema`).
- `check_duplicates` tool: pre-write duplicate check. Embeds candidate observations and queries for semantically similar existing observations (cosine > 0.80) on the same entity without writing anything. Returns per-candidate matches plus `modelReady: boolean` and `errorCount: number`. When `errorCount > 0`, some candidates hit errors (database corruption, embedding failure) and their empty matches should not be trusted. JSONL backend returns modelReady: false with empty matches and errorCount: 0.
- `add_observations` response includes optional `similarExisting` field with similarity warnings (cosine > 0.85). When `similarityCheckFailed: true` is set, the similarity check hit an error — absence of `similarExisting` means "check couldn't run," not "no duplicates."
- `get_connected_context` tool: bounded multi-hop graph traversal from a seed entity. Returns structure only (name/type/hopDistance/shortest-path — NO observation content) plus the active edges among the returned nodes, and mechanically-detected directed `cycles`. Surfaces INDIRECT facts that recency/semantic search miss (e.g. a sanction two hops from a loan applicant). Inputs: `seedEntity`, `maxHops` (1–6, default 3), `direction` ('out'|'in'|'both'), `maxNodes` (cap, default 50), `relationTypes` (case-insensitive type filter, e.g. `['CAUSED_BY']`), `projectId` (scope reached nodes to project+global; advisory), `asOf` (point-in-time). SQLite-only — JSONL throws. See the `getConnectedContext` JSDoc for the workload note (DEF-CG-01).
- `find_precedents` tool: similarity-RANKED observation retrieval (the most similar PRIOR decisions/situations to a query). Unlike `search_nodes` (recency-ordered) and `get_connected_context` (structural), it ranks by cosine MEANING and returns a score per result. Inputs: `query`, `projectId`, `memoryType` (recommend 'decision'), `limit` (default 5), `minSimilarity` (cosine floor, default 0.25 — gated on RAW cosine, rounded only for display). Degrades to `{ precedents: [], vectorSearchEnabled: false }` (no recency fallback) when the model/vec table is unavailable.
...
- **JSONL backend**: deprecated — no file locking, no FK validation, no `as_of` queries, no entity soft-delete, no `entity_timeline`, no `set_observation_metadata`, no eviction, no `check_duplicates` similarity matching (returns modelReady: false). Maintained only for environments without C build tools.
- **SQLite backend**: LIKE-based search is case-insensitive for ASCII only — non-ASCII Unicode characters (e.g. accented letters, CJK) may not match case-insensitively as expected. The FTS candidate list in relevance mode partially mitigates this (unicode61 tokenizer folds case + diacritics), but the LIKE list keeps the ASCII limitation
- **Relevance mode is top-k only** — `orderBy:'relevance'` returns the top `limit` fused results with `nextCursor: null`; combining it with `cursor` or `asOf` throws (rank is query-relative and mutation-unstable; the FTS index reflects current content only). JSONL backend cannot rank and falls back to recency order with `rankingUnavailable: true`
...
- **`memory_type` is free-form** — any string is accepted; there is no server-side validation of allowed values. Convention enforcement is the caller's responsibility. Known types: `decision`, `procedure`, `architecture`, `problem`, `preference`, `status`, `fact`, `emotional`. A typo in `memoryType` filter returns empty with a hint only when the string isn't in the known-types list.
- **`memoryType` + vector search for sparse types** — KNN retrieves top-k globally, then filters by `memory_type`. If a type has very few observations relative to the corpus, they may not appear in the global top-k and the vector search returns empty for that type. LIKE substring search is unaffected. Mitigated by using a larger k (200) when `memoryType` is set, but not eliminated for extremely sparse types at large scale.
...
- **v9→v10:** Source-instance provenance — adds `source_instance TEXT NOT NULL DEFAULT 'unknown'` on observations (which machine wrote it), backfilled to 'fedora'. Race-tolerant ALTER (concurrent server instances migrate in parallel — Issue #78 pattern).
- **v10→v11:** FTS5 full-text index — external-content `obs_fts` table over `observations.content` (`content_rowid='id'`, unicode61 + remove_diacritics 2) with three sync TRIGGERS (insert/delete/update-of-content; triggers rather than code-sync so evict.ts's raw-SQL writes stay covered). Index holds ALL observations — active-only filtering is the query-time `superseded_at=''` join. `'rebuild'` indexes the pre-v11 corpus; the migration transaction is IMMEDIATE with an in-transaction version re-check so racing instances (Issue #78 deployment) skip the corpus-sized rebuild instead of serially repeating it — IMMEDIATE is load-bearing: a DEFERRED re-check would read a pre-commit snapshot and die on SQLITE_BUSY_SNAPSHOT. NOTE: `bm25()` is an FTS5 auxiliary function usable only in a plain full-text query — aggregate contexts and flattened subqueries both throw; queries use a MATERIALIZED CTE (per-row scores inside, joins/aggregation outside).
- **v11→v12:** Retrieval strengthening — `access_count INTEGER NOT NULL DEFAULT 0` on entities. **PARITY RULE:** increments at exactly the sites that write `last_accessed_at` (touch helpers + createEntities' INSERT seeding 1 + the four inline `updated_at`+`last_accessed_at` UPDATEs) — one concept, two columns; a new `last_accessed_at` write site must increment `access_count` too. No backfill (DEFAULT 0 = never counted → `activationScore` −Infinity → excluded from the activation list). Race-tolerant ALTER (Issue #78 pattern).

## Tests (644 total across 13 test files)
- `__tests__/knowledge-graph.test.ts` (412 tests) — parameterized suite (`describe.each`) running shared behavioral tests against both `JsonlStore` and `SqliteStore`, covering all CRUD operations, deduplication, composite key safety, search, edge cases, observation timestamps, normalizeObservation validation, atomic writes (JSONL), idempotent delete edge cases, project filtering, cursor-based pagination, supersedeObservations, temporal relations (createdAt/supersededAt on relations, invalidateRelations, entityTimeline), similarity check, totalCount accuracy, entity name normalization behavioral tests, setObservationMetadata (importance/contextLayer/memoryType updates, entity not found, observation not found, surface variant lookup, timestamp bumps, superseded exclusion, batch updates), getContextLayers (empty result, importance sorting, layer filtering, project scoping, L2 exclusion, L1 token budget truncation), getSummary (empty result, importance+recency sorting, observation counts, limit param, project scoping, excludeContextLayers, aggregate stats, recentEntities project scoping, superseded observation exclusion, soft-deleted entity exclusion, memoryType filter on topObservations/recentEntities/stats), checkDuplicates (modelReady false when vectors off, empty matches for non-existent entity, multiple candidates), getConnectedContext (multi-hop traversal, maxHops bounding, cycle termination, direction, relationTypes filter, asOf, directed-cycle reporting incl. distinct same-concatenation cycles, maxNodes truncation, projectId scoping, out-of-project seed canonicalization + edge retention, truncation-honesty under scoping, foreign-intermediate path rendering, JSONL not-supported), findPrecedents degraded-path (empty + flags when vectors off, no recency fallback). Plus JSONL-specific and SQLite-specific tests (including `entity name normalization (Layer 1)` block, getSummary/getContextLayers soft-delete exclusion, setObservationMetadata batch atomicity rollback).
- `__tests__/mcp-tools.test.ts` (91 tests) — integration tests for MCP tool handlers including invalidate_relations, entity_timeline, normalization at the tool boundary, set_observation_metadata Zod schema validation (name normalization, importance range, contextLayer enum, null demotion), get_summary Zod schema validation (limit range, memoryType filter, valid input acceptance), search_nodes memoryType + orderBy parameter validation, check_duplicates Zod schema validation
- `__tests__/normalize-name.test.ts` (18 tests) — unit tests for `normalizeEntityName()`: NFC equivalence, case fold, separator stripping, C0 control-char stripping (incl. char(31) the traversal delimiter), Turkish dotted-I behavior, empty/whitespace/separator-only rejection
- `__tests__/relation-types.test.ts` (2 tests) — unit tests for the causal/precedent vocabulary: `CORE_RELATION_TYPES` membership/shape and `RELATION_TYPE_GUIDANCE` content (the three verbs + directionality)
- `__tests__/fts-search.test.ts` (24 tests) — includes Phase B activation pins (accessed-beats-equal-cold with hand-derived fusion math + recency-mode control; activation-never-nominates guard with positive control; weight-bound pin — raw-inserted cold entity, detects activation weight creeping past ~1.0; deterministic -Infinity-exclusion pin that catches a tail-ranking regression under BOTH FTS tie orders); plus: FTS5 availability probe (better-sqlite3 bundles FTS5 — if this fails, relevance mode's substrate is gone), v11 migration behavior (trigger sync, superseded-rows-stay-indexed invariant, rebuild), searchNodes relevance mode (rank inversion vs recency, top-k/nextCursor-null contract, projectId + memoryType filtering, superseded exclusion, cursor/asOf guard, explicit-recency pin, fractional-limit floor, FTS-only diacritic membership incl. punctuation-token survival, rankingDegraded present-on-loss/absent-on-healthy), toFtsQuery round-trip vs the real FTS5 parser (hostile corpus), trigger behavior under raw-SQL writes (Tier-1/Tier-2 eviction shapes), JSONL rankingUnavailable fallback + cursor guard. Pool 1 (vectors off — the degraded LIKE+FTS fusion path is itself the tested contract)
- `__tests__/rank-fusion.test.ts` (19 tests) — pure-function tests for `fuseRanks` (hand-derived RRF expectations incl. an off-rank-0-axis exact probe that kills mutant denominators, duplicated-id precondition pin, weight scaling), `toFtsQuery` (operator neutralization, quote doubling, zero-token drops, NUL stripping, empty input), `activationScore` (count/recency monotonicity, exact-value probe pinning d=0.5 and the ln form, never-accessed/-sentinel → -Infinity, clock-skew clamp). Model-free (Pool 1)
- `__tests__/precedent-ranking.test.ts` (5 tests) — pure-function tests for `rankAndFloorPrecedents` (the find_precedents ranking stage): floor gates on RAW cosine not the rounded display value, display rounding to 3 decimals, raw-DESC ordering preserved when two scores round equal, limit slice precedes round, empty when all below floor. Model-free (Pool 1) per the compute-expensive-testing rule.
...
- `__tests__/migration-validation.test.ts` (16 tests) — schema version tracking and migration validation across versions 1–11, including v7→v8 forward-migration test (display preservation + relation rewrite), collision-abort test (two active entities normalizing to same key → rollback), v8→v9 forward test (tombstoned_at + last_accessed_at columns with backfill verification), and v10→v11 forward test (obs_fts rebuild indexes pre-existing rows + all three triggers created). Convention: per-migration tests assert version "at least N"; the single EXACT current-version pin lives in knowledge-graph.test.ts's fresh-database test and is bumped deliberately with each migration
- `__tests__/eviction.test.ts` (19 tests) — eviction system tests: last_accessed_at access discipline (covering create/search/open/timeline/read/add/supersede operations), access_count parity-rule discipline (v12: create seeds 1, open/search increment, readGraph doesn't), eviction module unit tests (below-threshold no-op, getCapBytes env var + fallback, hard-delete tombstoned, tombstone superseded, LRU shield protection, active entity immunity), entity_timeline tombstoned status (observation + relation tombstoned rendering), eviction lifecycle integration
...
- `__tests__/vector-integration.test.ts` (12 tests) — end-to-end tests with real embedding model: semantic search, LIKE+vector dedup, superseded observation exclusion, similarExisting check, as_of recovery of superseded matches, soft-deleted entity exclusion, checkDuplicates similarity matching, checkDuplicates unrelated content rejection, find_precedents similarity-DESC ranking + memoryType + minSimilarity floor, find_precedents projectId scoping, relevance-mode vector-only semantic MEMBERSHIP (the ranked path's vector list, healthy-run flag absence) + its projectId scoping. Skip with `SKIP_VECTOR_INTEGRATION=1`.
...
**Test pool discipline:** The suite runs in two commands. Pool 1 (non-vector, 583 tests): `MEMORY_VECTOR_SEARCH=off SKIP_VECTOR_INTEGRATION=1 npx vitest run --exclude '**/vector-integration.test.ts'`. Pool 2 (vector integration, 10 tests): `MEMORY_VECTOR_SEARCH=on npx vitest run __tests__/vector-integration.test.ts --pool=forks --poolOptions.forks.singleFork=true`. See `feedback_local_llm_test_pool_discipline.md` for the reasoning (the local ONNX model load must be isolated and single-fork to avoid hard-reboots).

## Version History
- **v1.2.0** — Context-graph enhancements (no schema change; still v10). `get_connected_context` tool (bounded `WITH RECURSIVE` multi-hop traversal, structure-only, with mechanical directed-cycle detection); `find_precedents` tool (similarity-RANKED observation retrieval, floor gated on raw cosine); causal/precedent relation vocabulary (`relation-types.ts`: `CAUSED_BY` / `PRECEDENT_FOR` / `SUPERSEDES` + directionality guidance appended to `create_relations`); `delete_observations` converted to soft-delete (matches the preservation lifecycle); C0-control-char stripping in `normalize-name.ts` (guarantees char(31) — the traversal delimiter — can't enter a normalized name); `idx_relations_from_entity_active` index for the out-arm hop. 17 → 19 tools. 593 tests across 11 files. Design + deviations: `docs/superpowers/specs/2026-06-02-causal-relations-and-precedent-retrieval-design.md` (§9 + Appendix C deferment register: DEF-CG-01 deep-walk workload, DEF-CG-02 contextLayer/L2 coordination, DEF-CG-03 relation_type canonicalization).
- **v1.1.1** — `memoryType` filter on `search_nodes` and `get_summary` tools, `check_duplicates` pre-write duplicate detection tool, `busy_timeout = 5000` for concurrent access safety, `errorCount` on check_duplicates response, `similarityCheckFailed` on addObservations response, memoryType typo hints, larger KNN k for memoryType-filtered and check_duplicates queries. 545 tests across 9 files.
