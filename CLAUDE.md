...
- `toFtsQuery(raw)` — FTS5 MATCH sanitizer: wraps every whitespace token in double quotes (embedded `"` doubled) so operators in user input (AND/OR/NOT/NEAR/*/^/-) match literally. User input must NEVER reach MATCH unsanitized
...
- `__tests__/fts-search.test.ts` (12 tests) — FTS5 availability probe (better-sqlite3 bundles FTS5 — if this fails, relevance mode's substrate is gone), v11 migration behavior (trigger sync, superseded-rows-stay-indexed invariant, rebuild), searchNodes relevance mode (rank inversion vs recency, top-k/nextCursor-null contract, projectId + memoryType filtering, superseded exclusion, cursor/asOf guard), JSONL rankingUnavailable fallback. Pool 1 (vectors off — the degraded LIKE+FTS fusion path is itself the tested contract)
- `__tests__/rank-fusion.test.ts` (14 tests) — pure-function tests for `fuseRanks` (hand-derived RRF expectations, weight scaling), `toFtsQuery` (operator neutralization, quote doubling, empty input), `activationScore` (count/recency monotonicity, never-accessed/-sentinel → -Infinity, clock-skew clamp). Model-free (Pool 1)
